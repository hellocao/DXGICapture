...
protected:
  ...
  int fMaxNumSockets; //windows下面这个参数完全可以忽略
  fd_set fReadSet;
  fd_set fWriteSet;    //connect连接成功或者可非阻塞发送数据
  fd_set fExceptionSet;//connnect连接失败

private:
	int fDummySocketNum; // Hack to work around a bug in Windows' "select()" implementation:感觉没什么用
};
#endif
